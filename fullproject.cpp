...
Bukti daftarBukti[JUMLAH_BUKTI] = {
    {"SURAT ANCAMAN", "Surat ancaman ditemukan di meja kerja David. Tulisan tangan menyerupai Liam.", false},
    {"REKAMAN CCTV KORIDOR", "Liam terlihat berjalan menuju kantor David sekitar pukul 22.05.", false},
    {"DRAFT EMAIL DAVID", "David berencana melaporkan Liam atas dugaan penyalahgunaan dana perusahaan.", false},
    {"SIDIK JARI PADA BOTOL", "Sidik jari Liam ditemukan pada botol minuman korban.", false},
    {"HASIL FORENSIK", "Racun tidak ditemukan pada botol minuman. Racun ditemukan pada es batu.", false},
    {"CCTV DAPUR", "Lena terlihat berada sendirian di dekat freezer beberapa saat sebelum pesta dimulai.", false},
    {"DOKUMEN AYAH LENA", "Ditemukan surat pemecatan ayah Lena yang ditandatangani David bertahun-tahun lalu.", false}
};
...
void menuWawancara() {
    ...
    do {
        ...
        for (int i = 0; i < JUMLAH_TERSANGKA; i++) {
            int terjawab = 0;
            for (int q = 0; q < JUMLAH_PERTANYAAN; q++)
                if (tersangka[i].pernahDitanya[q]) terjawab++;
            cout << (i + 1) << ". " << tersangka[i].nama
                 << "  [" << terjawab << "/" << JUMLAH_PERTANYAAN << " pertanyaan]\n";
        }
        cout << "0. Kembali\n";
        garis();
...
void catatanDetektif() {
    ...
    for (int i = 0; i < JUMLAH_TERSANGKA; i++)
        if (tersangka[i].sudahWawancara) wawancaraSelesai++;

    cout << "Progres wawancara : " << wawancaraSelesai << "/" << JUMLAH_TERSANGKA << " tersangka diperiksa\n";
    cout << "Status bukti      : " << (semuaBuktiDianalisis ? "Sudah dianalisis" : "Belum dianalisis") << "\n";
    garis();
...
void investigasi() {
    ...
    do {
        ...
        switch (pilih) {
            case 1: menuWawancara(); break;
            case 2: analisisBukti(); break;
            case 0: clear(); break;
            default:
...
void tampilEndingGagal() {
    ...
    garis();
    cout << "\n";
    cout << "   Tuduhan Anda meleset terlalu jauh. Tekanan publik memaksa\n";
    cout << "   kasus ini ditutup tanpa kepastian.\n\n";
    cout << "   Pelaku sebenarnya, LENA, menyimpan racun itu pada es batu\n";
    cout << "   di gelas David - balas dendam atas ayahnya yang difitnah.\n";
    cout << "   Ia melenggang pergi, dan keadilan tak pernah datang.\n\n";
    cout << "   Seorang detektif sejati membaca bukti, bukan menebak.\n\n";
    garis();
    cout << "\n";
...
bool tentukanPelaku() {
    ...
    for (int i = 0; i < JUMLAH_TERSANGKA; i++) {
        if (!tersangka[i].sudahWawancara) semuaWawancara = false;
        for (int q = 0; q < JUMLAH_PERTANYAAN; q++)
            if (tersangka[i].pernahDitanya[q]) totalPertanyaan++;
    }
    ...
    while (sisaKesempatan > 0) {
        ...
        if (pilihan == PELAKU_BENAR) {
            ...
            garis();
            skor += (sisaKesempatan == 2 ? 50 : 25); // bonus jika tepat di percobaan pertama
            tertangkap = true;
            pause();
            break;
    ...
    cout << "1. Melalui botol minuman\n";
    cout << "2. Melalui makanan saat makan malam\n";
    cout << "3. Melalui es batu di dalam gelasnya\n";
    cout << "4. Melalui asap rokok\n";
    garis();
    ...
    cout << "1. Ingin mewarisi seluruh harta David\n";
    cout << "2. Menutupi kasus penggelapan dana\n";
    cout << "3. Balas dendam atas ayahnya yang difitnah David\n";
    cout << "4. Cintanya yang ditolak oleh David\n";
    garis();
    ...
    cout << "Skor akhir          : " << HIJAU << BOLD << skor << RESET << "\n";
    cout << "Peringkat           : " << CYAN << BOLD << rank << RESET << "\n";
    garis();
